...
pub(super) fn primitive_of(schema: &Schema) -> Option<&Primitive> {
...
pub(super) mod sqlite {
    ...
    fn bind_all<'q>(
        ...
        for (value, _schema) in args {
            query = match value {
                // sqlite is dynamically typed; any None binds a plain NULL
    ...
    pub fn decode_row(row: &SqliteRow, target: &Schema) -> Result<Value, DatabaseError> {
        match target.base() {
...
pub(super) mod postgres {
    ...
    pub fn decode_row(row: &PgRow, target: &Schema) -> Result<Value, DatabaseError> {
        match target.base() {
            Schema::Record(fields) => fields
                .iter()
                .map(|(name, schema)| {
        ...
        I: sqlx::ColumnIndex<PgRow> + std::fmt::Display + Copy,
    {
        ...
        // postgres is strictly typed per column, so integer/float targets
        // accept either width and convert
        macro_rules! get {
            ($ty:ty) => {
