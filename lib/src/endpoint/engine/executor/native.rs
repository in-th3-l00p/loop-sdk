...
mod tests {
	...
	async fn call_runs_handler_off_the_async_thread() {
		...
		assert_eq!(call(handler, vec![Value::I64(21)]).await.unwrap(), Value::I64(42));
	}

	#[tokio::test]
	async fn call_surfaces_handler_panics_as_errors() {
		let handler: Arc<dyn Handler> = Arc::new(|_: &[Value]| -> Result<Value, HandlerError> {
			panic!("kaboom")
		});
	...
	async fn subscribe_bridges_iterator_to_channel_in_order() {
		let source: Arc<dyn Source> = Arc::new(|_: &[Value]| -> Result<crate::endpoint::ValueStream, HandlerError> {
	...
	async fn subscribe_propagates_mid_stream_errors() {
		let source: Arc<dyn Source> = Arc::new(|_: &[Value]| -> Result<crate::endpoint::ValueStream, HandlerError> {
			Ok(Box::new(vec![Ok(Value::I64(1)), Err::<Value, HandlerError>("lost".into())].into_iter()))
		});
