	...
	signature
		...
		.map(|param| {
			let value = if let Some(raw) = path.get(&param.name) {
				...
				codec::json::decode(&param.schema, json)
					.map_err(|e| EngineError::Decode(DecodeError(format!("parameter {:?}: {e}", param.name))))?
...
fn decode_scalar(name: &str, schema: &Schema, raw: &str) -> Result<Value, EngineError> {
	let error = |expected: &str| {
		EngineError::Decode(DecodeError(format!("parameter {name:?}: expected {expected}, found {raw:?}")))
	};

	let Schema::Primitive(primitive) = schema else {
		return Err(error("a primitive (path/query parameters cannot be lists or maps)"));
	};
	...
	match primitive {
		...
		Primitive::Blob => BASE64.decode(raw).map(Value::Blob).map_err(|_| error("base64 blob"))
	}
}
...
mod tests {
	...
	fn signature(params: Vec<(&str, Schema)>) -> Signature {
		Signature {
			params: params
				.into_iter()
				.map(|(name, schema)| Parameter { name: name.into(), schema })
				.collect(),
	...
	fn rejects_non_primitive_query_parameter() {
		let signature = signature(vec![("xs", Schema::List(Box::new(Schema::Primitive(Primitive::I64))))]);
	...
	fn decodes_structured_body_parameter() {
		...
		assert_eq!(args, vec![Value::List(vec![Value::I64(1), Value::I64(2), Value::I64(3)])]);
	}
}
