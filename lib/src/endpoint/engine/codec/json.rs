...
fn decode_primitive(primitive: &Primitive, json: &Json) -> Result<Value, DecodeError> {
	match primitive {
		Primitive::Bool => json.as_bool().map(Value::Bool).ok_or_else(|| mismatch("bool", json)),
		Primitive::I32 => json
			.as_i64()
		...
		Primitive::I64 => json.as_i64().map(Value::I64).ok_or_else(|| mismatch("i64", json)),
		Primitive::U64 => json.as_u64().map(Value::U64).ok_or_else(|| mismatch("u64", json)),
		Primitive::F32 => json.as_f64().map(|n| Value::F32(n as f32)).ok_or_else(|| mismatch("f32", json)),
		Primitive::F64 => json.as_f64().map(Value::F64).ok_or_else(|| mismatch("f64", json)),
		Primitive::Str => json.as_str().map(|s| Value::Str(s.into())).ok_or_else(|| mismatch("str", json)),
		Primitive::Date => json.as_str().map(|s| Value::Date(s.into())).ok_or_else(|| mismatch("date", json)),
		Primitive::Blob => {
			let s = json.as_str().ok_or_else(|| mismatch("base64 blob", json))?;
			BASE64.decode(s).map(Value::Blob).map_err(|e| DecodeError(format!("invalid base64 blob: {e}")))
		}
	}
}
...
fn decode_map(key: &Schema, value: &Schema, json: &Json) -> Result<Value, DecodeError> {
	if matches!(key, Schema::Primitive(Primitive::Str)) {
		...
		object
			.iter()
			.map(|(k, v)| {
				let decoded = decode(value, v).map_err(|e| DecodeError(format!("key {k:?}: {e}")))?;
		...
		entries
			...
			.map(|(i, entry)| {
				let Json::Array(pair) = entry else {
					return Err(DecodeError(format!("entry {i}: expected [key, value] pair")));
				};
				let [k, v] = pair.as_slice() else {
					return Err(DecodeError(format!("entry {i}: expected exactly 2 elements")));
				};
				Ok((
...
pub fn encode(value: &Value) -> Json {
	match value {
		...
		Value::F32(n) => Number::from_f64(f64::from(*n)).map(Json::Number).unwrap_or(Json::Null),
		...
		Value::Map(entries) => {
			if entries.iter().all(|(k, _)| matches!(k, Value::Str(_))) {
				...
				Json::Array(entries.iter().map(|(k, v)| Json::Array(vec![encode(k), encode(v)])).collect())
			}
		}
	}
}
...
mod tests {
	...
	fn decodes_primitives_per_schema() {
		assert_eq!(decode(&Schema::Primitive(Primitive::I64), &json!(5)).unwrap(), Value::I64(5));
		assert_eq!(decode(&Schema::Primitive(Primitive::F64), &json!(5)).unwrap(), Value::F64(5.0));
		assert_eq!(
	...
	fn decodes_string_keyed_map_from_object() {
		...
		assert_eq!(decoded, Value::Map(vec![(Value::Str("a".into()), Value::I64(1))]));
	}
	...
	fn decodes_non_string_keyed_map_from_pairs() {
		...
		assert_eq!(
			decoded,
			Value::Map(vec![(Value::I64(1), Value::Bool(true)), (Value::I64(2), Value::Bool(false))])
		);
	}
	...
	fn roundtrips_nested_value_through_json() {
		...
		let value = Value::Map(vec![(Value::Str("xs".into()), Value::List(vec![Value::I64(1), Value::I64(2)]))]);
