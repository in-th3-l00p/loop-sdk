...
async fn rest_endpoint_serves_json_over_http() {
	let add = Endpoint {
		...
		signature: Signature {
			params: vec![
				Parameter { name: "a".into(), schema: i64_schema() },
				Parameter { name: "b".into(), schema: i64_schema() },
			],
			output: i64_schema()
		},
	...
	assert_eq!(response.json::<serde_json::Value>().await.unwrap(), json!(42));
}
...
async fn sse_endpoint_streams_data_frames() {
	let ticks = Endpoint {
		name: "ticks".into(),
		signature: Signature { params: vec![], output: i64_schema() },
		access: Access::Sse { url: "/ticks".into() },
		binding: Binding::Stream(Arc::new(|_: &[Value]| -> Result<ValueStream, HandlerError> {
...
async fn live_endpoint_pushes_values_over_websocket() {
	let feed = Endpoint {
		name: "feed".into(),
		signature: Signature {
			params: vec![Parameter { name: "from".into(), schema: i64_schema() }],
			output: i64_schema()
		},
		access: Access::Live { url: "/feed".into() },
		binding: Binding::Stream(Arc::new(|args: &[Value]| -> Result<ValueStream, HandlerError> {
			let [Value::I64(from)] = args else { return Err("bad args".into()) };
			let from = *from;
	...
	let (mut socket, _) =
		tokio_tungstenite::connect_async(format!("ws://{addr}/feed?from=10")).await.unwrap();
