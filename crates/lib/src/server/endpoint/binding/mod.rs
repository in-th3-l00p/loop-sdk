...
mod tests {
    ...
    use crate::server::endpoint::Context;

    #[test]
    fn native_binding_dispatches_through_handler_trait_object() {
        let binding = Binding::Native(Arc::new(|_: &Context, args: &[Value]| match args {
        ...
        let result = handler
            .call(&Context::default(), &[Value::I64(2), Value::I64(3)])
            .unwrap();
    ...
    fn native_binding_propagates_handler_errors() {
        let binding = Binding::Native(Arc::new(
            |_: &Context, _: &[Value]| -> Result<Value, HandlerError> { Err("boom".into()) },
        ));

        let Binding::Native(handler) = &binding else {
            unreachable!()
        };
        assert!(handler.call(&Context::default(), &[]).is_err());
    }
